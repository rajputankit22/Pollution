...
void loop() {
 ...
 /* logfile = SD.open("test139.csv", FILE_WRITE);
  ...
  logfile.flush();
 */
 mF = SD.open("TRAIL111.csv",FILE_WRITE);
 myFile = SD.open("TRAIL999.csv",FILE_WRITE);
if(myFile){
  ...
  DateTime now = rtc.now();
    ...
    myFile.print('/');
    mF.print(now.year(),DEC);
    mF.print('/');
    Serial.print(now.month(),DEC);
    Serial.print('/');
    myFile.print(now.month(),DEC);
    myFile.print('/');
    mF.print(now.month(),DEC);
    mF.print('/');
    ...
    myFile.print(",");
    mF.print(now.day(),DEC);
    mF.print(",");
    Serial.print(now.hour(),DEC);
    Serial.print(':');
    myFile.print(now.hour(),DEC);
    myFile.print(':');
    mF.print(now.hour(),DEC);
    mF.print(':');
    Serial.print(now.minute(),DEC);
    Serial.print(':');
    myFile.print(now.minute(),DEC);
    myFile.print(':');
    mF.print(now.minute(),DEC);
    mF.print(':');
    Serial.print(now.second(),DEC);
    myFile.println(now.second(),DEC);
    mF.print(now.second(),DEC);
    Serial.print("@");
    myFile.print(",");
    mF.print(",");
...
//MQ7
  myFile1 = SD.open("TRAIL888.csv",FILE_WRITE);
  if(myFile1){
     DateTime now = rtc.now();
...
//DUST SENSOR
  myFile2 = SD.open("TRAIL777.csv",FILE_WRITE);
 if(myFile2){
   DateTime now = rtc.now();
...
*/
 myFile3 = SD.open("TRAIL666.csv",FILE_WRITE);
 if(myFile3){
   DateTime now = rtc.now();
...
//2SH12
myFile4 = SD.open("TRAIL555.csv",FILE_WRITE);
  if(myFile4){
     DateTime now = rtc.now();
...
{
  if ( gas_id == GAS_LPG ) {
     return MQGetPercentage(rs_ro_ratio,ro,LPGCurve);
  }  else if ( gas_id == GAS_SMOKE ) {
     return MQGetPercentage(rs_ro_ratio,ro,SmokeCurve);
  }    
