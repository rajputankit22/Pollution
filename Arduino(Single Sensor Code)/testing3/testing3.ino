...
//#define         GAS_CO                       (1)
#define         GAS_SMOKE                    (1)
long MQ135_RO=68550; //default Ro for MQ135_DEFAULTPPM ppm of CO2
...
Serial.println("Date \t\tTime \t\tNO2 \tCO2 \tAlcohol \tCO \tDUST \tLPG \tSmoke");
 }


void loop() {
 myFile = SD.open("test139.csv",FILE_WRITE);
 if(myFile){
...
MQ135_RO= (resvalue * exp( log(MQ135_SCALINGFACTOR/ppm1) / MQ135_EXPONENT ));
...
/*-----------------------------------------------*/
 ...
 myFile3 = SD.open("test333.csv",FILE_WRITE);
 if(myFile3){
 
 // Serial.print("sensor_volt = ");
  //Serial.print(sensor_volt);
  //Serial.println("V");
   myFile3.print(sensor_volt);
...
long mq135_getro(long resvalue,long ppm1) {
  return (long)(resvalue * exp( log(MQ135_SCALINGFACTOR/ppm1) / MQ135_EXPONENT ));
}
...
double mq7_getppm(long resvalue1,long default_RO1) 
{
  double ret = 0;
  double validinterval = 0;
  validinterval = resvalue1/(double)default_RO1;
  //if(validinterval<MQ7_MAXRSRO && validinterval>MQ7_MINRSRO) {
    ret = (double)MQ7_SCALINGFACTOR * pow( ((double)resvalue1/default_RO1), MQ7_EXPONENT);
  //}
  return ret;
}
