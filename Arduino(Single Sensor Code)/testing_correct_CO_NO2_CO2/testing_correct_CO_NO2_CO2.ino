...
void loop() {
 ...
 /* logfile = SD.open("test139.csv", FILE_WRITE);
  ...
  logfile.flush();
 */myFile = SD.open("test531.csv",FILE_WRITE);
if(myFile){
...
//MQ7
  myFile1 = SD.open("testmqco.csv",FILE_WRITE);
  if(myFile1){
     DateTime now = rtc.now();
...
myFile1.close();
 ...
 {// if the file didn't open, print an error:
    Serial.println("error opening testmqco.txt");
  } 

  
 
//DUST SENSOR
  myFile2 = SD.open("dust.csv",FILE_WRITE);
 if(myFile2){
   DateTime now = rtc.now();
...
/*-----------------------------------------------*/
 ...
 myFile3 = SD.open("lpgsmoke.csv",FILE_WRITE);
 if(myFile3){
   DateTime now = rtc.now();
